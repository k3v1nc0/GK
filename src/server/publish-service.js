...
function normalizeGameMinimapMarkerCategory(raw, fallback = {}, index = 0) {
  ...
  return {
    ...
    fixedSizeOnZoom: raw?.fixedSizeOnZoom !== undefined ? raw.fixedSizeOnZoom === true : fallback.fixedSizeOnZoom === true,
...
function normalizeGameMinimapMarkerCategories(values, defaults, rawValues = {}) {
  ...
  const seen = new Set();
  const normalizedCategories = rawCategories.map(function (category, index) {
    const fallback = defaultCategories.find(function (defaultCategory) {
      return defaultCategory?.id === category?.id || defaultCategory?.source === category?.source;
    }) || defaultCategories[index] || {};
    const normalized = normalizeGameMinimapMarkerCategory(category, fallback, index);
    if (legacyMode) {
      if (normalized.source === "character") normalized.enabled = values.showLocalPlayer !== false;
      if (normalized.source === "users") normalized.enabled = values.showRemotePlayers !== false;
  ...
  const hasCharacter = normalizedCategories.some(function (category) {
    return category.id === "character" || category.source === "character";
  });
  const defaultCharacter = defaultCategories.find(function (category) {
    return category?.id === "character" || category?.source === "character";
  });
  if (!hasCharacter && defaultCharacter) {
    const usersCategory = normalizedCategories.find(function (category) {
      return category.id === "users" || category.source === "users";
    });
    const character = normalizeGameMinimapMarkerCategory(Object.assign({}, defaultCharacter, usersCategory || {}, {
      id: "character",
      label: defaultCharacter.label || "Character/User",
      source: "character",
      enabled: usersCategory ? usersCategory.enabled !== false : values.showLocalPlayer !== false
    }), defaultCharacter, 0);
    return [character].concat(normalizedCategories);
  }
...
function buildGameMinimapHudReadModel(node) {
  ...
  return {
    ...
    showLocalPlayer: gameMinimapCategoryEnabled(markerCategories, "character"),
