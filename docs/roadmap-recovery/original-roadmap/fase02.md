---
source_path: README/fase2.md
source_commit: 2a9a6077510237e8dfc5c638d0ca996b67a5fa05
source_title: Fase 2 - Serverfundering onder /var/www/gk
archived_for: roadmap recovery / deep research
---

# Fase 2 - Serverfundering onder /var/www/gk
...
Bereid de single-server omgeving voor met repo, assets, data, releases, secrets, logs, MySQL, Redis, Nginx en systemd.

## Wat Kevin vooraf moet maken, kiezen of samen uitwerken

- Bevestig `/var/www/gk`.
- Bevestig domein/subpad voor game en editor.
- Zet bestaande assets onder de afgesproken assets-map.

## Actie voor Codex

Maak mappen, users, rechten, secrets, MySQL, Redis, Nginx en systemd templates actief op de server.
...
Je werkt aan fase 2: Serverfundering onder /var/www/gk.

Doel:
Bereid de single-server omgeving voor met repo, assets, data, releases, secrets, logs, MySQL, Redis, Nginx en systemd.

Werk uit:
Maak ops scripts en templates voor `/var/www/gk`: create-runtime-dirs, check-host, check-assets, nginx config, systemd units, env examples en server-layout docs. Scripts mogen geen secrets bevatten.
...
- [ ] Mappen bestaan.
- [ ] Assets-map wordt gevonden.
- [ ] Secrets buiten Git.
- [ ] Nginx serveert geen secrets/data.
- [ ] Codex kan checks draaien.

## Testplan

Codex draait create-runtime-dirs, check-host en check-assets op de server.
