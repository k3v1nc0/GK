...
Fase 5.3 houdt Kevin toegang tot de bestaande GameBibleNode-bestanden, maar maakt onderscheid tussen publieke leesroutes, browser-save en beschermde schrijfacties. Fase 5.2 bewees dat de save-route veilig kon werken; Fase 5.3 voegt de ontbrekende normale editor-admin login/session flow toe.

## Bronnen

De server moet deze GameBibleNode-bestanden functioneel houden:

- `README/GameBibleNode.html`
- `README/GameBibleNode.json`
- `README/GameBibleNode.php`

In deze repo-werkset staat de legacy PHP-wrapper in Git. De HTML/JSON kunnen server-side onder `/var/www/gk/README` aanwezig zijn; wanneer ze niet in Git staan, blijft dit document het repo-contract voor Codex serverconfiguratie en smoke-tests.
...
Fase 5.3 koppelt deze route aan de echte editor session:

- `POST /auth/editor/login` zet de editor session cookie en CSRF-cookie;
- `GET /auth/editor/me` bevestigt `editor_admin`;
- `POST /editor/game-bible-node/save` leest dezelfde editor session cookie;
- een game session of anonieme browser krijgt geen save-toegang.
