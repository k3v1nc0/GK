import { createGkWorldRuntime } from "../shared/world-runtime.js?v=20260921-authoring-convergence2";
...
function shouldSendDebugHudPointerToGame(event) {
  if (!event || state.hudLayout.editMode === true) return false;
  if (event.pointerType !== "mouse" || event.button !== 0) return false;
  const target = event.target;
  if (!target || typeof target.closest !== "function") return false;
  const debugPanel = target.closest('.gameHudPanelFrame[data-runtime-family="debug"]');
...
function handleGameHudPointerDown(event) {
  if (shouldSendDebugHudPointerToGame(event)) {
    handleGamePointerDown(event, "debug-hud");
    return;
  }
  if (state.hudLayout.editMode !== true) return;
...
const GAME_MINIMAP_MARKER_SOURCE_SET = new Set([
  "character", "users", "enemy", "boss", "wildlife", "npc", "quest", "spawn", "teleport",
...
function legacyGameMinimapMarkerCategories(config) {
  return [
    { id: "character", label: "Character/User", source: "character", enabled: config?.showLocalPlayer !== false, color: "#ffe08a", shape: "triangle", showLabel: config?.showPlayerName !== false, clampOutside: true, showThroughFog: true, iconSizePx: num(config?.iconSizePx, 9), fontSizePx: num(config?.fontSizePx, 10), nameMaxLength: num(config?.nameMaxLength, 14), animation: "none" },
    { id: "users", label: "Users", source: "users", enabled: config?.showRemotePlayers !== false, color: "#ffe08a", shape: "triangle", showLabel: config?.showPlayerName !== false, clampOutside: true, showThroughFog: true, iconSizePx: num(config?.iconSizePx, 9), fontSizePx: num(config?.fontSizePx, 10), nameMaxLength: num(config?.nameMaxLength, 14), animation: "none" },
...
function gameMinimapMarkerCategories(config = resolveGameMinimapConfig()) {
  ...
  const seen = new Set();
  const categories = raw.map(normalizeGameMinimapMarkerCategory).filter(function (category) {
  ...
  if (!hasCharacter) {
    ...
    const character = normalizeGameMinimapMarkerCategory(Object.assign({
      id: "character",
      label: "Character/User",
      source: "character",
      enabled: config?.showLocalPlayer !== false,
      color: "#ffe08a",
      shape: "triangle",
      showLabel: config?.showPlayerName !== false,
      clampOutside: true,
      showThroughFog: true,
      iconSizePx: num(config?.iconSizePx, 9),
      fontSizePx: num(config?.fontSizePx, 10),
      nameMaxLength: num(config?.nameMaxLength, 14),
      animation: "none"
    }, usersCategory || {}, {
      id: "character",
      label: "Character/User",
      source: "character",
      enabled: usersCategory ? usersCategory.enabled !== false : config?.showLocalPlayer !== false
    }));
...
function drawGameMinimapCategoryMarker(ctx, category, marker, viewBounds, size, performanceMode, now, markerScale) {
  ...
  if (!point) return;
  const categoryScale = category.fixedSizeOnZoom === true ? 1 : markerScale;
  const iconWidth = clamp(num(category.iconSizePx, 9) * categoryScale, 1.5, 64);
  const iconSize = iconWidth / 2;
  const fontSize = clamp(num(category.fontSizePx, 10) * categoryScale, 2, 32);
  ...
  if ((category.showLabel === true || category.shape === "label") && performanceMode !== "ultra" && fontSize >= 4) {
    drawMarkerLabel(ctx, marker.label || category.label || marker.id, point.x, point.y, fontSize, nameMaxLength, iconSize + Math.max(2, 3 * categoryScale), 2);
  }
}
...
function handleGamePointerDown(event, source = "pointer") {
  ...
  if (hasMovementInput()) {
    noteLocalControlStart(true, source || "pointer");
    sendInputState({ force: true });
  } else {
    clearPointerTarget(false);
  }
...
function releaseGamePointer(event) {
  if (!state.pointer.active && state.pointer.pointerId === null && state.pointer.sprintPointerId === null) return;
  if (isNode03Defeated()) {
...
function bindPointerControls() {
  ...
  canvas.addEventListener("pointerdown", function (event) {
    handleGamePointerDown(event, "pointer");
  });
  canvas.addEventListener("pointermove", handleGamePointerMove);
  window.addEventListener("pointermove", function (event) {
    if (!event.defaultPrevented) handleGamePointerMove(event);
  });
  canvas.addEventListener("pointerup", releaseGamePointer);
  canvas.addEventListener("pointercancel", releaseGamePointer);
  canvas.addEventListener("lostpointercapture", releaseGamePointer);
  window.addEventListener("pointerup", function (event) {
    if (!event.defaultPrevented) releaseGamePointer(event);
  });
