...
const state = {
  ...
  selectedNodeIds: [],
  selectedEdgeIds: [],
  authoringSelectedModelNodeId: null,
  clipboard: null,
  marquee: null,
  ...
  if (targetExists) {
    ...
    item.addEventListener("click", function () {
      if (nodeId) {
        ...
        const runtimeId = runtimeNodeId(nodeById(nodeId));
        if (runtimeId && runtime && typeof runtime.frameEntity === "function") {
          requestAnimationFrame(function () { runtime.frameEntity(runtimeId); });
        }
      } else if (edgeId) {
...
function runtimeNodeId(node) {
  ...
  ].includes(node.type)) return node.id || null;
  return null;
}
...
function normalizeSelectionState() {
  ...
  if (state.authoringSelectedModelNodeId) {
    const authoringNode = state.graph.nodes.find(function (node) {
      return node.id === state.authoringSelectedModelNodeId && node.type === "model_entity";
    }) || null;
    if (!authoringNode) state.authoringSelectedModelNodeId = null;
  }
}
...
function setSelection(nodeIds, edgeIds, options = {}) {
  ...
  if (options.clearAuthoringSelection) {
  ...
  renderInspector();
  if (!options.preserveRuntimeSelection) syncRuntimeSelection();
...
function clearSelection(options = {}) {
  setSelection([], [], {
    clearPendingEdge: options.clearPendingEdge,
    clearAuthoringSelection: options.clearAuthoringSelection,
    preserveRuntimeSelection: options.preserveRuntimeSelection
  });
}
...
async function boot() {
  ...
  runtime = createGkWorldRuntime(el.viewportCanvas, {
    ...
    onRenderFrame: function () {
      if (state.authoringIndicatorsEnabled) syncAuthoringIndicatorScreenPositions();
    },
...
function selectedViewportAuthoringNode() {
  ...
  const authoringModel = selectedAuthoringModelNode(state.graph);
  if (authoringModel) return authoringModel;
  return null;
}

function selectedAuthoringModelNode(graph = state.graph) {
  const runtimeId = runtimeSelectedEntityId();
  if (runtimeId) {
    const runtimeNode = nodeByRuntimeId(runtimeId);
    if (runtimeNode?.type === "model_entity") return runtimeNode;
  }
  const selectedNode = state.selectedNodeId ? graphNodeByIdInGraph(graph, state.selectedNodeId) : null;
  if (selectedNode?.type === "model_entity"
    && Array.isArray(state.selectedNodeIds)
    && state.selectedNodeIds.length === 1
    && state.selectedNodeIds[0] === selectedNode.id) {
    return selectedNode;
  }
  return state.authoringSelectedModelNodeId
    ? (graphNodeByIdInGraph(graph, state.authoringSelectedModelNodeId) || null)
    : null;
}

function selectedSingleModelNode() {
  const node = selectedAuthoringModelNode(state.graph);
  return node?.type === "model_entity" ? node : null;
}
...
function objectFunctionBeginDraft(kind, context) {
  ...
  if (kind === "quest") {
    const existingEntityRef = normalizeCanonicalId(existing?.values?.entityRef || "", "");
...
function objectFunctionBuildNodeValuesForKind(kind, context, draft, existingNode, graph = state.graph) {
  ...
  if (kind === "interaction") {
    ...
    const existingEntityRef = normalizeCanonicalId(existingNode?.values?.entityRef || "", "");
    const derivedEntityRef = normalizeCanonicalId(context.assembly?.values?.entityId || objectFunctionAssemblyEntityId(context.model, graph), "");
    next.entityRef = existingNode && !existingEntityRef ? null : (existingEntityRef || derivedEntityRef);
...
function objectFunctionNextGraphMutation(context, draft) {
  ...
  if (kind === "interaction" || kind === "npc" || kind === "enemy") {
    ...
    if (node) {
      objectFunctionRemoveEdges(nextGraph, function (edge) {
        return edge.fromNodeId === nextContextModel.id
          && edge.fromPort === "entity"
...
function renderQuestTimelineView(group) {
  ...
  const selectedRelationIds = selectedModelQuestDialogueDefinitionIds(state.graph);
...
function renderQuestDialogueWorkspace(group) {
  ...
  if (selectedModel) {
    const selectedInfo = selectedModelAuthoringReferenceInfo(state.graph);
    const relatedCount = selectedModelQuestDialogueDefinitionIds(state.graph).size;
    const context = document.createElement("div");
    context.className = "objectFunctionDraftHint";
    context.textContent = "Geselecteerd object: " + nodeDisplayTitle(selectedModel)
      + " · " + relatedCount + " quest/dialoog-verwijzing(en)"
      + (selectedInfo.questBindings?.length
        ? " via " + selectedInfo.questBindings.map(function (binding) { return binding.values?.targetId || nodeDisplayTitle(binding); }).join(", ")
        : "");
...
async function createRootManagedGroup(kind, titleText) {
  ...
  if (duplicate) {
    enterGroup(duplicate, { preserveAuthoringSelection: true });
  ...
  await restoreGraphObject(nextGraph, {
    ...
    refreshValidation: true,
    afterApply: function () {
      enterGroup(group, { preserveAuthoringSelection: true });
...
function linkedQuestTargetPatchesForModelTransform(model, transformPatch, graph = state.graph, excludedNodeIds = new Set()) {
  if (!model || model.type !== "model_entity") return [];
  const values = {};
  for (const key of ["x", "y", "z"]) {
    if (!Object.prototype.hasOwnProperty.call(transformPatch || {}, key)) continue;
    const number = Number(transformPatch[key]);
    if (Number.isFinite(number)) values[key] = number;
  }
  if (!Object.keys(values).length) return [];
  return questTargetBindingsForModel(model, graph).filter(function (binding) {
    return binding && !excludedNodeIds.has(binding.id);
  }).map(function (binding) {
    return { nodeId: binding.id, values: values };
  });
}
...
function selectedModelAuthoringReferenceInfo(graph = state.graph) {
  return modelAuthoringReferenceInfo(selectedAuthoringModelNode(graph), graph);
}
...
function selectedModelQuestDialogueDefinitionIds(graph = state.graph) {
  const selected = selectedModelAuthoringReferenceInfo(graph);
  if (!selected.model || !selected.refs.size) return new Set();
  return questDialogueDefinitionIdsForRefs(selected.refs, graph);
}

function selectedObjectRelationBadge(labelText = "Geselecteerd object") {
  const badge = document.createElement("span");
  badge.className = "authoringSelectionBadge";
...
function renderWorldZoneHub() {
  ...
  for (const zoneGroup of zones) {
    ...
    open.addEventListener("click", function () { enterGroup(zoneGroup, { preserveAuthoringSelection: true }); });
...
function authoringIndicatorScaleForScreen(screen) {
  const distance = Number(screen?.distance);
  if (!Number.isFinite(distance) || distance <= 0) return 1;
  return Math.max(0.52, Math.min(1, 28 / distance));
}

function positionAuthoringIndicatorCluster(cluster, screen, wrapRect) {
  if (!cluster || !screen || !wrapRect || !Number.isFinite(screen.x) || !Number.isFinite(screen.y)) return false;
  cluster.style.left = Math.round(screen.x - wrapRect.left) + "px";
  cluster.style.top = Math.round(screen.y - wrapRect.top - 18) + "px";
  cluster.style.setProperty("--indicator-scale", authoringIndicatorScaleForScreen(screen).toFixed(2));
  return true;
}

function syncAuthoringIndicatorScreenPositions() {
  const root = el.viewportAuthoringIndicators;
  if (!root || root.hidden || !state.authoringIndicatorsEnabled || !runtime || typeof runtime.worldToScreen !== "function") return;
  const wrapRect = el.viewportWrap?.getBoundingClientRect();
  if (!wrapRect) return;
  for (const cluster of root.querySelectorAll(".authoringIndicatorCluster")) {
    const x = Number(cluster.dataset.worldX);
    const y = Number(cluster.dataset.worldY);
    const z = Number(cluster.dataset.worldZ);
    if (!Number.isFinite(x) || !Number.isFinite(y) || !Number.isFinite(z)) {
      cluster.hidden = true;
      continue;
    }
    const screen = runtime.worldToScreen({ x, y, z });
    cluster.hidden = !positionAuthoringIndicatorCluster(cluster, screen, wrapRect);
  }
}
...
function renderAuthoringIndicators() {
  ...
  for (const model of models) {
    ...
    if (!Number.isFinite(x) || !Number.isFinite(z)) continue;
    const anchorY = Number.isFinite(y) ? y + 1.2 : 1.2;
    const cluster = document.createElement("div");
    cluster.className = "authoringIndicatorCluster";
    cluster.dataset.worldX = String(x);
    cluster.dataset.worldY = String(anchorY);
    cluster.dataset.worldZ = String(z);
    ...
    if (visibleCount) {
      const screen = runtime.worldToScreen({ x, y: anchorY, z });
      cluster.hidden = !positionAuthoringIndicatorCluster(cluster, screen, wrapRect);
...
function focusSelectedViewportAuthoringNode() {
  ...
  requestAnimationFrame(function () {
    ...
    if (model) {
      selectNode(model.id, true, { clearPendingEdge: true, showMobileInspector: true });
      return;
    }
...
function openAuthoringWorkspaceNode(node) {
  if (!node) return;
  ensureMobileAllLayout();
  enterGroup(node, { preserveAuthoringSelection: true });
}
...
function openRootAuthoringWorkspace() {
  ...
  state.currentGroupId = null;
  clearSelection({ clearPendingEdge: true, preserveRuntimeSelection: true });
  syncBreadcrumb();
  renderGraph();
  renderInspector();
  applyTransform();
...
async function commitGroupTransform(payload) {
  ...
  for (const entry of commits) {
    ...
    if (!node || node.type !== "model_entity") continue;
    const values = normalizeModelEntityTransformPatch(node, entry.transform || {});
    patchedNodeIds.add(node.id);
    patches.push({ nodeId: node.id, values: values });
    for (const targetPatch of linkedQuestTargetPatchesForModelTransform(node, values, state.graph, patchedNodeIds)) {
      patchedNodeIds.add(targetPatch.nodeId);
...
// ---------- Groups + breadcrumb ----------
function enterGroup(node, options = {}) {
  state.currentGroupId = node.id;
  clearSelection({
    clearPendingEdge: true,
    preserveRuntimeSelection: options.preserveAuthoringSelection === true,
    clearAuthoringSelection: options.clearAuthoringSelection === true
  });
  state.view = { panX: 40, panY: 40, scale: 1 };
  syncBreadcrumb();
  renderGraph();
...
function buildZoneCanvasInspectorBlock(group) {
  ...
  open.addEventListener("click", function () { enterGroup(group, { preserveAuthoringSelection: true }); });
...
async function patchValues(nodeId, patch, options = {}) {
  ...
  return await applyGraphMutation(async function () {
    if (localValuePatch) {
      const bulkPatches = runtimeTransformPatch
        ? [{ nodeId: nodeId, values: cleanPatch }].concat(linkedQuestTargetPatchesForModelTransform(node, cleanPatch))
        : [];
      if (bulkPatches.length > 1) {
        await apiOk("/api/editor/nodes/values/bulk", {
          method: "POST",
          body: JSON.stringify({ patches: bulkPatches, returnGraph: false })
        });
        return graphWithPatchedNodeValuesBulk(state.graph, bulkPatches);
      }
